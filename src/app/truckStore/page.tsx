...
const TrucksTableModal: React.FC<TrucksTableModalProps> = ({ isOpen, onClose, selectedProgramId, liters, purchase_id }) => {
  ...
  useEffect(() => {
    if (selectedProgramId && programDetails) {
      ...
      const fetchProgramDetails = async () => {
        try {
          const response = await axios.get(`https://tms.sdssn.org/api/programs/${selectedProgramId}`);
...
console.log("Program Details:", programDetails);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('https://tms.sdssn.org/api/marketers/program-trucks', payload, {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 max-w-lg relative">
        <button className="absolute top-3 right-3 text-lg text-black" onClick={onClose}>✖</button>
        <h2 className="text-2xl font-bold mb-4 text-center text-black">Select a Truck</h2>

        {loading && <p className="text-center text-blue-500">Loading trucks...</p>}
          ...
          <div>
            <ul className="space-y-2">
              {trucks.map((truck) => (
                <li
                  key={truck.id}
                  className={`p-3 border rounded cursor-pointer hover:bg-gray-100 text-black ${selectedTruck?.id === truck.id ? 'bg-gray-200' : ''}`}
            ...
            {selectedTruck && (
              <div className="mt-4 p-4 bg-gray-100 rounded">
                <p className='text-center text-black'><strong>Selected Truck:</strong> {selectedTruck.name}</p>
                <p className='text-center text-black'><strong>Driver:</strong> {selectedTruck.driver ? `${selectedTruck.driver.first_name} ${selectedTruck.driver.last_name}` : 'N/A'}</p>
              </div>
            )}
